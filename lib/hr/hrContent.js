...
export async function createJobApplication(application) {
  ...
  formData.set('jobId', application.jobId || '');
  formData.set('jobTitle', application.jobTitle || 'General Application');
  formData.set('applicantName', application.applicantName?.trim() || '');
  formData.set('email', application.email?.trim() || '');
  formData.set('phone', application.phone?.trim() || '');
  formData.set('coverLetter', application.coverLetter?.trim() || '');

  if (application.resumeFile) {
    formData.set('resumeFile', application.resumeFile);
  }
  ...
  return mapApplicationRow(result.application);
}
...
export async function deleteJobApplication(id, resumeUrl = '', options = {}) {
  const requestedByRole = options.requestedByRole || options.role || '';

  if (!isSuperAdminRole(requestedByRole)) {
    throw new Error('Only the super admin can delete applicant records.');
  }
