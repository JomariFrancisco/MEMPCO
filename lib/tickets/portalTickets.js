...
const TICKET_COLUMNS_WITHOUT_STATUS_HISTORY = TICKET_COLUMNS.replace(/\s*status_history,\s*/m, '');
const TICKET_LIST_COLUMNS_WITHOUT_STATUS_HISTORY = TICKET_LIST_COLUMNS.replace(/\s*status_history,\s*/m, '');
...
const isTicketBeingHandled = (ticket = {}) => {
  ...
  return (
    ...
    ['in progress', 'moved date', 'escalated'].includes(status)
  );
};
...
const canTicketAcceptMessages = (ticket = {}) => {
  ...
  if (['resolved', 'canceled', 'cancelled'].includes(status)) {
    return false;
  }
...
export async function updateTicket(ticketId, updates = {}, options = {}) {
  try {
    ...
    const { data, error } = await runTicketQuery(
      ...
      TICKET_INSERT_RETURN_COLUMNS_WITHOUT_STATUS_HISTORY,
...
export async function createTicketMessage(ticketId, options = {}) {
  try {
    ...
    if (!canTicketAcceptMessages(currentTicket)) {
      throw new Error(
        'Conversation is available only while ICT is actively handling the ticket. Resolved or canceled tickets are closed for replies.'
      );
    }
