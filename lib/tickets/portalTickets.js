...
import { deriveTicketImpact, normalizeSlaLevel } from '@/lib/tickets/sla';
...
const stripOptionalTicketColumns = (columns) =>
  ['status_history', 'burnout_report', 'serial_number', 'recommendation'].reduce(
    (nextColumns, column) => nextColumns.replace(new RegExp(`\\s*${column},\\s*`, 'm'), ''),
    columns
  );
const TICKET_COLUMNS_WITHOUT_STATUS_HISTORY = stripOptionalTicketColumns(TICKET_COLUMNS);
const TICKET_LIST_COLUMNS_WITHOUT_STATUS_HISTORY = stripOptionalTicketColumns(TICKET_LIST_COLUMNS);
const TICKET_INSERT_RETURN_COLUMNS_WITHOUT_STATUS_HISTORY = TICKET_LIST_COLUMNS_WITHOUT_STATUS_HISTORY;
...
const isTicketBeingHandled = (ticket = {}) => {
  const status = normalizeTicketStatus(ticket.status);

  return (
    hasAssignedTechnician(ticket) ||
    Boolean(ticket.workStartedAt || ticket.work_started_at) ||
    [
      'in progress',
      'moved date',
      'escalated',
...
const canTicketAcceptMessages = (ticket = {}) => {
  const status = normalizeTicketStatus(ticket.status);
  ...
  return isTicketBeingHandled(ticket);
};
...
const getEmployeeLockDetails = (ticket = {}) => {
  ...
  if (status === 'resolved') {
  ...
  } else if (status === 'passed burnout') {
    employeeLockReason = 'This Burnout ticket has already passed and was completed by ICT.';
  } else if ([
    'ready for deployment',
    'deployed',
    'failed burnout',
    ...
    employeeLockReason = 'This Burnout ticket has already been completed by ICT.';
    ...
    employeeLockReason = 'ICT already recorded action, remarks, resolution, or recommendation for this ticket.';
  }
...
const isMissingStatusHistoryColumnError = (error) => {
  const message = String(error?.message || '').toLowerCase();

  return (
    error?.code === '42703' ||
    error?.code === 'PGRST204' ||
    message.includes('status_history') ||
    message.includes('statushistory') ||
    message.includes('burnout_report') ||
    message.includes('burnoutreport') ||
...
const runTicketQuery = async (buildQuery, columns, fallbackColumns, timeoutMessage) => {
  const result = await withTimeout(buildQuery(columns), timeoutMessage);

  if (!result.error || !isMissingStatusHistoryColumnError(result.error) || !fallbackColumns) {
    return result;
  }

  return withTimeout(buildQuery(fallbackColumns), timeoutMessage);
};

const withoutStatusHistoryPayload = (payload = {}) => {
  const nextPayload = { ...payload };
  delete nextPayload.status_history;
  delete nextPayload.burnout_report;
...
const mapTicketRow = (row = {}) => {
  const ticket = {
    ...
    burnoutReport: row.burnout_report && typeof row.burnout_report === 'object' ? row.burnout_report : null,
...
const getTicketById = async (ticketId, { full = true } = {}) => {
  ...
  const columns = full ? TICKET_COLUMNS : TICKET_LIST_COLUMNS;
  const fallbackColumns = full
    ? TICKET_COLUMNS_WITHOUT_STATUS_HISTORY
    : TICKET_LIST_COLUMNS_WITHOUT_STATUS_HISTORY;

  const { data, error } = await runTicketQuery(
    (selectColumns) =>
      supabase
        .from('tickets')
...
export async function createTicket({ user, form }) {
  try {
    ...
    const {
      ...
      'Session lookup took too long. Please log in again.'
    );
    ...
    if (!response.ok) {
      throw new Error(result.error || 'Unable to create ticket in Supabase.');
    }

    return mapTicketRow(result.ticket);
  } catch (error) {
...
export async function updateTicket(ticketId, updates = {}, options = {}) {
  try {
    ...
    const sendPatchRequest = (requestUpdates) =>
      withTimeout(
        fetch('/api/tickets', {
          method: 'PATCH',
          headers: {
    ...
    let response = await sendPatchRequest(nextUpdates);

    const result = await response.json().catch(() => ({}));

    if (!response.ok) {
      const missingRecommendation = String(result.error || '').toLowerCase().includes('recommendation');

      if (missingRecommendation && nextUpdates.recommendation !== undefined) {
        const fallbackUpdates = { ...nextUpdates };
        delete fallbackUpdates.recommendation;
        response = await sendPatchRequest(fallbackUpdates);
        const fallbackResult = await response.json().catch(() => ({}));

        if (!response.ok) {
          throw new Error(fallbackResult.error || 'Unable to update ticket details.');
        }

        return mapTicketRow(fallbackResult.ticket);
      }

      throw new Error(result.error || 'Unable to update ticket details.');
    }
...
export async function createTicketMessage(ticketId, options = {}) {
  try {
    ...
    if (!canTicketAcceptMessages(currentTicket)) {
      throw new Error(
        'Conversation is available only while ICT is actively handling the ticket. Closed, canceled, or final Burnout records are closed for replies.'
      );
    }
...
export function subscribeToTickets(onTicketChange, onStatusChange) {
