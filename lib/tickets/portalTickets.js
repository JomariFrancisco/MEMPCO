...
const stripOptionalTicketColumns = (columns) =>
  ['status_history', 'burnout_report', 'serial_number'].reduce(
    ...
    const { data, error } = await runTicketQuery(
      (selectColumns) =>
        supabase
          .from('tickets')
          .update(
            selectColumns === TICKET_INSERT_RETURN_COLUMNS_WITHOUT_STATUS_HISTORY
              ? withoutStatusHistoryPayload(payload)
              : payload
          )
          .eq('id', ticketId)
          .select(selectColumns)
          .single(),
      TICKET_INSERT_RETURN_COLUMNS,
      payload.burnout_report ? null : TICKET_INSERT_RETURN_COLUMNS_WITHOUT_STATUS_HISTORY,
