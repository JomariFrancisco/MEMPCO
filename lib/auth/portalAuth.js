...
const DESIGNATION_PROFILE_COLUMNS = `${BASE_PROFILE_COLUMNS}, designation`;
const PROFILE_COLUMNS = `${DESIGNATION_PROFILE_COLUMNS}, profile_photo`;
...
const uploadProfilePhoto = async (supabase, userId, profilePhoto = '') => {
  if (!profilePhoto || !String(profilePhoto).startsWith('data:')) return profilePhoto;

  const blob = dataUrlToBlob(profilePhoto);
  ...
  const path = `${userId}/${photoId}.${getImageExtension(blob.type)}`;
  const { error } = await supabase.storage.from(PROFILE_PHOTO_BUCKET).upload(path, blob, {
    cacheControl: '3600',
...
const isMissingProfilePhotoColumnError = (error) =>
  error?.code === '42703' ||
  error?.code === 'PGRST204' ||
  error?.message?.toLowerCase().includes('profile_photo');
...
const runProfileQuery = async (buildQuery, timeoutMessage) => {
  ...
  if (isMissingProfilePhotoColumnError(result.error)) {
    const designationResult = await withTimeout(
      buildQuery(DESIGNATION_PROFILE_COLUMNS),
      timeoutMessage
    );

    if (!designationResult.error || !isMissingDesignationColumnError(designationResult.error)) {
      return designationResult;
    }
  } else if (!isMissingDesignationColumnError(result.error)) {
    return result;
  }
...
export const mapProfileToPortalUser = (profile, authUser = null) => {
  ...
  return {
    ...
    profilePhoto: profile?.profile_photo || metadata.profile_photo || '',
    status: profile?.status || 'Active',
    mustChangePassword: Boolean(
      appMetadata.must_change_password ?? metadata.must_change_password
    ),
...
export const isInactivePortalUser = (user) =>
  ['inactive', 'locked'].includes(String(user?.status || '').trim().toLowerCase());
...
export async function updateCurrentPortalUserProfilePhoto(profilePhoto = '') {
  const supabase = createClient();
  const {
    data: { user },
    error: authError,
    ...
    supabase.auth.getUser(),
    'Unable to verify your active session.'
  );

  if (authError || !user?.id) {
    throw normalizeAuthRequestError(
      authError || new Error('Please sign in again before updating your profile photo.')
    );
  }

  const storedProfilePhoto = await uploadProfilePhoto(supabase, user.id, profilePhoto);

  const { error: updateError } = await withTimeout(
    supabase.rpc('update_own_profile_photo', {
      photo_url: storedProfilePhoto,
    }),
    'Unable to save your profile photo.'
  );

  if (updateError) {
    throw normalizeProfileError(updateError);
  }
  ...
  return mapProfileToPortalUser(data, user);
}

export async function refreshPortalUserPassword(id) {
  const result = await requestJson(
...
export async function listPortalUserAuditLogs(userId) {
  if (!userId) return [];
  ...
  const result = await requestJson(
    ...
    'Unable to load user audit trail.'
  );
  ...
  return (result.logs || []).map((log) => ({
    ...
    actorName: log.actor_name || 'Super Admin',
    action: log.action || 'updated',
    summary: log.summary || 'Account updated.',
    changes: log.changes || {},
...
export async function updatePortalPassword(password) {
  ...
  await requestJson(
    '/api/auth/change-password',
    {
      ...
      body: JSON.stringify({
        ...
        confirmPassword: parsedPassword,
      }),
    },
  ...
  await withTimeout(
    ...
    'Session refresh took too long. Please log in again.',
    5000
  ).catch(() => null);
...
export async function completeRequiredPasswordChange(password, confirmPassword) {
  await requestJson(
    '/api/auth/change-password',
    {
      method: 'POST',
